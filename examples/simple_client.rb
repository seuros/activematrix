...
class SimpleClient < ActiveMatrix::Client
  ...
  def on_message(room, event)
    case event.type
    when 'm.room.member'
      puts "[#{Time.zone.now.strftime '%H:%M'}] #{event[:content][:displayname]} joined." if event.membership == 'join'
    when 'm.room.message'
      ...
      if %w[m.text m.notice].include? event.content[:msgtype]
        ...
        puts "[#{Time.zone.now.strftime '%H:%M'}] <#{prefix}#{user.display_name}> #{"\033[1;30m" if notice}#{event.content[:body]}#{"\033[0m" if notice}"
      elsif event[:content][:msgtype] == 'm.emote'
        puts "[#{Time.zone.now.strftime '%H:%M'}] *#{prefix}#{user.display_name} #{event.content[:body]}"
      else
        puts "[#{Time.zone.now.strftime '%H:%M'}] <#{prefix}#{user.display_name}> (#{event.content[:msgtype]}) #{event.content[:body]} - #{api.get_download_url event.content[:url]}"
      end
    end
  end
end
